...
1. **Bugfixes**
  ...
  1. Error still happens when deleting a line near a header (tested with a line
  above and one below, currenly have only been able to replicate it in level 2 headers
  and only in some files; it also seemed to happen when using `<S-j>` to join lines in
  a situation where the line below the cursor (to be joined with the current line) was
  above a header):
  ...
  A similar error happens if the file ends with a header, followed by a blank
  line, followed by a single line of text, and then the line above or below the header is
  deleted. If the line of text immediately follows the header (without a blank line inbetween), the error
  still happens when deleting the line above the header.

  ```
  Error in decoration provider "line" (ns=nvim.treesitter.highlighter):
  Error executing lua: ...64/share/nvim/runtime/lua/vim/treesitter/highlighter.lua:370: Invalid 'end_col': out of range
  stack traceback:
          [C]: in function 'nvim_buf_set_extmark'
          ...64/share/nvim/runtime/lua/vim/treesitter/highlighter.lua:370: in function 'fn'
          ...64/share/nvim/runtime/lua/vim/treesitter/highlighter.lua:232: in function 'for_each_highlight_state'
  ...
  No error seems to happen if the header is followed by a line of text, which
  is in turn followed by a line (even if blank), regardless of there being a
  blank line between the header and the first line of text.

  If all lines below the header are blank, there seems to be no error. And if there are multiple lines
  of text after the header, an error occurs.

  If a header is followed by text, with or without blank lines inbetween, then
  followed by another header, then deleting the second header causes the error,
  even if this second header is followed by text. Putting a blank line at the
  end of the file seems to protect against this error as well, but I don't want
  to have "meaningless" blank lines at the end of files unless strictly necessary.
...
2. **Improvements**:
   ...
   2. My current config (maybe from Kanagawa, maybe from my user configs) show
   the line numbers for markdown files (on the left). This does not seem to be
   useful for note editing, so we can consider removing it while in "PKM mode".
